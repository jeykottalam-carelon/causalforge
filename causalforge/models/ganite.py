"""
Code Skeleton from: https://github.com/vanderschaarlab/mlforhealthlabpub/blob/main/alg/ganite/ganite/ganite.py
"""
...
class Ganite_Model(nn.Module):
    ...
    def __init__(
        ...
        super(Ganite_Model, self).__init__()
        ...
        # Hyperparameters
        dim_hidden = 100 if 'dim_hidden' not in stub.params else stub.params['dim_hidden']
        minibatch_size =  256 if 'minibatch_size' not in stub.params else stub.params['minibatch_size']
        alpha =  0.1 if 'alpha' not in stub.params else stub.params['alpha']
        beta =  0 if 'beta' not in stub.params else stub.params['beta']
        depth =  0 if 'depth' not in stub.params else stub.params['depth']
        num_iterations =  5000 if 'num_iterations' not in stub.params else stub.params['num_iterations']
        num_discr_iterations =  1 if 'num_discr_iterations' not in stub.params else stub.params['num_discr_iterations']
